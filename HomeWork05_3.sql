...
WHERE 
	...
	AND 
    (
		(u.id = fr.initiator_user_id AND fr.target_user_id = 1)
		OR  
		(u.id = fr.target_user_id    AND fr.initiator_user_id = 1)
    )
ORDER BY u.id;
...
CREATE OR REPLACE VIEW mess AS 
SELECT 
	id AS 'ID_mess', 
	body
FROM messages
...
CREATE OR REPLACE VIEW about_users AS
SELECT 
	u.firstname,
    u.lastname,
    p.gender, 
...
SELECT 
	CONCAT(u.firstname, ' ', u.lastname) AS 'user_name',
    COUNT(m.from_user_id) AS 'count_messeges',
    DENSE_RANK() OVER(ORDER BY COUNT(m.from_user_id) DESC) AS 'rank'
FROM users u
JOIN messages m
WHERE u.id = m.from_user_id
GROUP BY m.from_user_id
ORDER BY `count_messeges` DESC;
...
SELECT *,
    CONCAT(
        (CASE
			WHEN 
				TIME_FORMAT(LEAD(created_at) OVER (), '%H:%i:%S') < TIME_FORMAT(created_at, '%H:%i:%S') 
			THEN DATEDIFF(LEAD(created_at) OVER (), created_at) - 1
			ELSE DATEDIFF(LEAD(created_at) OVER (), created_at)
		END), 
		' days ',
    SEC_TO_TIME(
		TIMESTAMPDIFF(
